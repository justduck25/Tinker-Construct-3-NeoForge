...
public class FluidEffectProvider extends AbstractFluidEffectProvider {
  ...
  protected void addFluids() {
    RegistryHelper.setFallbackRegistryAccess(lookupProvider.join());
    // cheap //
    // water
    ...
    FluidMobEffect returningEffect = new FluidMobEffect(holder(TinkerEffects.returning), 7*20, 1, null);
    ...
    addFluid(TinkerFluids.venom, FluidValues.SIP).addEffect(FluidMobEffect.builder().effect(MobEffects.POISON, 20 * 5).effect(MobEffects.STRENGTH, 20 * 10), TimeAction.ADD);
    ...
    addMetal(TinkerFluids.moltenIron).fireDamage(2f).addEffect(FluidMobEffect.builder().effect(holder(TinkerEffects.magnetic), 20 * 5, 2), TimeAction.SET);
    addMetal(TinkerFluids.moltenSteel).fireDamage(2f).addEffect(FluidMobEffect.builder().effect(holder(TinkerEffects.repulsive), 20 * 5, 2), TimeAction.SET);
    // zinc/brass - speed/heat
    compatMetal(TinkerFluids.moltenZinc).fireDamage(2).addEffect(FluidMobEffect.builder().effect(MobEffects.SPEED, 20 * 10), TimeAction.SET);
    ...
    compatMetal(TinkerFluids.moltenPewter, "tin", "lead").fireDamage(3).addEffect(FluidMobEffect.builder().effect(MobEffects.STRENGTH, 20 * 7), TimeAction.SET);
    ...
    compatMetal(TinkerFluids.moltenElectrum, "silver").magicDamage(3).addEffect(FluidMobEffect.builder().effect(MobEffects.HASTE, 20 * 8, 1), TimeAction.SET);
    ...
    compatMetal(TinkerFluids.moltenNicrosil).magicDamage(3).addEffect(FluidMobEffect.builder().effect(holder(TinkerEffects.experienced), 20 * 5, 1), TimeAction.SET);
    ...
    compatMetal(TinkerFluids.moltenPlatinum).fireDamage(2).addEffect(FluidMobEffect.builder().effect(MobEffects.MINING_FATIGUE, 20 * 10, 1), TimeAction.SET);
    compatMetal(TinkerFluids.moltenTungsten).fireDamage(2).addEffect(FluidMobEffect.builder().effect(MobEffects.SLOWNESS, 20 * 10, 2), TimeAction.SET);
    compatMetal(TinkerFluids.moltenOsmium).fireDamage(2).addEffect(FluidMobEffect.builder().effect(MobEffects.SLOWNESS, 20 * 10, 2), TimeAction.SET);
    ...
    // tinkers nether
    addMetal(TinkerFluids.moltenCobalt).fireDamage(3).addEffect(FluidMobEffect.builder().effect(MobEffects.HASTE, 20 * 7, 1).effect(MobEffects.SPEED, 20 * 7, 1), TimeAction.SET);
    addMetal(TinkerFluids.moltenManyullyn).fireDamage(4).addEffect(FluidMobEffect.builder().effect(holder(TinkerEffects.bleeding), 20 * 3, 1), TimeAction.SET);
    ...
    // tinkers end
    addMetal(TinkerFluids.moltenKnightmetal).spikeDamage(3).addEffect(FluidMobEffect.builder().effect(holder(TinkerEffects.pierce), 20 * 5, 2), TimeAction.SET);
    // thermal compat
    ...
    compatMetal(TinkerFluids.moltenLumium).magicDamage(4).addEffect(FluidMobEffect.builder().effect(MobEffects.GLOWING, 20 * 5, 1).effect(MobEffects.SPEED, 20 * 5, 1).effect(MobEffects.JUMP_BOOST, 20 * 5, 1), TimeAction.SET);
    compatMetal(TinkerFluids.moltenEnderium).magicDamage(4).addEffect(FluidMobEffect.builder().effect(holder(TinkerEffects.enderference), 20 * 10, 1), TimeAction.SET);
    // mekanism compat
    ...
    compatMetal(TinkerFluids.moltenRefinedObsidian).spikeDamage(3).addEffect(FluidMobEffect.builder().effect(holder(TinkerEffects.bleeding), 20 * 2, 1), TimeAction.SET);
    ...
    compatFluid("plantoil", 50)
      ...
      .addEntityEffects(FluidMobEffect.builder().effect(MobEffects.NAUSEA, 5 * 20, 1).buildEntity(TimeAction.ADD));
    {
      String ie = "immersiveengineering";
      MobEffect flammable = FakeRegistryEntry.effect(Identifier.parse(ie + ":flammable"));
      Holder<MobEffect> flammableHolder = BuiltInRegistries.MOB_EFFECT.wrapAsHolder(flammable);
      compatFluid(ie, "creosote",  50)
        .addEffect(FluidMobEffect.builder().effect(flammableHolder, 8 * 20, 1), TimeAction.ADD)
        .addEntityEffect(new FireFluidEffect(TimeAction.ADD, 8));
      compatFluid(ie, "biodiesel", 50)
        .addEffect(FluidMobEffect.builder().effect(flammableHolder, 8 * 20, 2), TimeAction.ADD)
        .addEntityEffect(new FireFluidEffect(TimeAction.ADD, 8));
      MobEffect conductiveEffect = FakeRegistryEntry.effect(Identifier.parse(ie + ":conductive"));
      Holder<MobEffect> conductiveHolder = BuiltInRegistries.MOB_EFFECT.wrapAsHolder(conductiveEffect);
      FluidMobEffect conductive = new FluidMobEffect(conductiveHolder, 8 * 20, 2);
    ...
    compatMetal(TinkerFluids.moltenSteeleaf).magicDamage(2).addEffect(FluidMobEffect.builder().effect(holder(TinkerEffects.experienced), 20 * 5, 1), TimeAction.SET);
  ...
  private Enchantment enchantment(ResourceKey<Enchantment> key) {
    var reg = lookupProvider.join().lookupOrThrow(Registries.ENCHANTMENT);
    Enchantment ench = reg.getOrThrow(key).value();
    TinkerEnchantmentLoadable.registerKey(ench, key);
    BaseRegistryLoadable.registerKey(ench, key.identifier());
    return ench;
  }

  /** Casts a holder to a wildcard compatible holder type */
  @SuppressWarnings("unchecked")
  private static <T> Holder<T> holder(Holder<? extends T> holder) {
