...
public class FluidContainerTransferProvider extends AbstractFluidContainerTransferProvider {
  ...
  protected void addTransfers() {
    addTransfer("potion_bucket_empty", new EmptyPotionTransfer(Ingredient.of(TinkerFluids.potion.asItem()), ItemOutput.fromItem(Items.BUCKET), TinkerFluids.potion.result(FluidType.BUCKET_VOLUME)));
    addTransfer("potion_bucket_fill", new FillPotionBucketTransfer(Ingredient.of(Items.BUCKET), ItemOutput.fromItem(TinkerFluids.potion.asItem()), TinkerFluids.potion.ingredient(FluidType.BUCKET_VOLUME)));
  ...
  protected void addContainerlessEmpty(String name, String domain, FluidOutput fluid) {
    ...
    addTransfer(domain + '_' + name, new EmptyFluidContainerTransfer(ItemNameIngredient.from(id), ItemOutput.EMPTY, fluid), new RegisteredCondition<>(ResourceKey.create(Registries.ITEM, id)));
  }

  @Override
  public String getName() {
    return "Continuum Construct Fluid Container Transfer";
  }
}
