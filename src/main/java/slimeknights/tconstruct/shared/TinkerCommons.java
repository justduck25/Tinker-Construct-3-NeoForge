...
public final class TinkerCommons extends TinkerModule {
  ...
  public static final ItemObject<Block> soulGlass = BLOCKS.register("soul_glass", () -> new SoulGlassBlock(glassBuilder(MapColor.COLOR_BROWN).speedFactor(0.2F).noCollision().isViewBlocking((state, getter, pos) -> true)), TOOLTIP_BLOCK_ITEM);
   ...
   * Items
   */
  public static final ItemObject<EdibleItem> bacon = ITEMS.register("bacon", () -> new EdibleItem(itemProps().food(TinkerFood.BACON)));
  public static final ItemObject<EdibleItem> jeweledApple = ITEMS.register("jeweled_apple", () -> new EdibleItem(itemProps().food(TinkerFood.JEWELED_APPLE, TinkerFood.JEWELED_APPLE_CONSUMABLE)));
  public static final ItemObject<Item> cheeseIngot = ITEMS.register("cheese_ingot", () -> new CheeseItem(itemProps().food(TinkerFood.CHEESE)));
  public static final ItemObject<Block> cheeseBlock = BLOCKS.register("cheese_block", () -> new HalfTransparentBlock(builder(MapColor.COLOR_YELLOW, SoundType.HONEY_BLOCK).strength(1.5F, 3.0F).speedFactor(0.4F).jumpFactor(0.5F).noOcclusion()), block -> new CheeseBlockItem(block, itemProps().food(TinkerFood.CHEESE)));
  ...
  public static final DeferredHolder<ParticleType<?>, ParticleType<FluidParticleData>> fluidParticle = PARTICLE_TYPES.register("fluid", () -> new FluidParticleData.Type(TConstruct.getResource("fluid")));

    /* Loot conditions are registered through the active NeoForge/Mantle MapCodec registries. */
  ...
  public static final BlockContainerOpenedTrigger CONTAINER_OPENED_TRIGGER = BlockContainerOpenedTrigger.CONTAINER;
  public static final ToolInventoryChangeTrigger TOOL_INVENTORY_CHANGED_TRIGGER = ToolInventoryChangeTrigger.TOOL_INVENTORY_CHANGED;
  ...
  void registerRecipeSerializers(RegisterEvent event) {
    ...
    if (event.getRegistryKey() == Registries.RECIPE_SERIALIZER) {
      CriteriaTriggers.register("tconstruct:container_opened", CONTAINER_OPENED_TRIGGER);
      CriteriaTriggers.register("tconstruct:tool_inventory_changed", TOOL_INVENTORY_CHANGED_TRIGGER);
  ...
  @SubscribeEvent
  void gatherClientData(final GatherDataEvent.Client event) {
    DataGenerator generator = event.getGenerator();
    PackOutput output = generator.getPackOutput();
    generator.addProvider(true, new ModelSpriteProvider(output, event.getResourceManager(PackType.CLIENT_RESOURCES)));
    generator.addProvider(true, new TinkerSpriteSourceProvider(output, event.getLookupProvider()));
    generator.addProvider(true, new TinkerItemModelProvider(output));
    generator.addProvider(true, new TinkerBlockStateProvider(output));
    generator.addProvider(true, new RenderFluidProvider(output));
    generator.addProvider(true, new RenderItemProvider(output));
  }

  @SubscribeEvent
  void gatherServerData(final GatherDataEvent.Server event) {
    DataGenerator generator = event.getGenerator();
    PackOutput output = generator.getPackOutput();
    generator.addProvider(true, new CommonRecipeProvider.Runner(output, event.getLookupProvider()));
  }
