...
public interface TinkerSuggestionProvider {
  ...
  static Identifier toIdentifier(Object id) {
    if (id instanceof Identifier location) {
      return location;
    }
    return Identifier.parse(String.valueOf(id));
  }  /**
   ...
   */
  static <T> void filterResources(String defaultDomain, Iterable<T> resources, String input, Function<T, ?> idGetter, Consumer<T> resultConsumer) {
    boolean hasNamespace = input.indexOf(':') > -1;
    for(T resource : resources) {
      Identifier location = toIdentifier(idGetter.apply(resource));
   ...
   */
  static <T> CompletableFuture<Suggestions> suggestResource(String defaultDomain, Iterable<T> resources, SuggestionsBuilder builder, Function<T, ?> idGetter, Function<T,Message> tooltip) {
    String remaining = builder.getRemaining().toLowerCase(Locale.ROOT);
    filterResources(defaultDomain, resources, remaining, idGetter, (resource) -> builder.suggest(toIdentifier(idGetter.apply(resource)).toString(), tooltip.apply(resource)));
    return builder.buildFuture();
  }
   ...
   * @param tooltip         Function mapping elements to their tooltip
   */
  static <T> CompletableFuture<Suggestions> suggestResource(String defaultDomain, Stream<T> resources, SuggestionsBuilder builder, Function<T, ?> idGetter, Function<T, Message> tooltip) {
