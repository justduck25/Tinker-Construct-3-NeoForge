...
public record MobEquipment(EquipmentSlot slot, IJsonPredicate<Item> match, ItemOutput tool, List<RandomMaterial> materials, @Nullable TagKey<Fluid> fluid, float chance, int priority) {
  ...
  public static boolean apply(List<MobEquipment> replace, Mob mob, @Nullable Object event) {
    ...
    if (!apply.isEmpty()) {
      // NeoForge 26 spawn-finalize event signatures changed; apply replacements directly after spawn.
      ...
      if (replacement.is(TinkerTags.Items.MODIFIABLE)) {
        ...
        if (fluid != null) {
          ...
          if (amount > 0) {
            ...
            List<Holder<Fluid>> fluids = java.util.stream.StreamSupport.stream(BuiltInRegistries.FLUID.getTagOrEmpty(this.fluid).spliterator(), false).toList();
            Fluid fluid = fluids.isEmpty() ? Fluids.EMPTY : fluids.get(random.nextInt(fluids.size())).value();
